...
const App = () => {
	...
	const addTodo = async todo => {
		await axios.post("http://localhost:4002/api/v1/todos", {
	...
	const fetchTodos = async () => {
		const res = await axios.get('http://localhost:4002/api/v1/todos');
	...
	const editTodo = async (id, newContent) => {
		await axios.patch(`http://localhost:4002/api/v1/todos/${id}`, {
			content: newContent,
		});
	...
	const deleteTodo = async id => {
		await axios.delete(`http://localhost:4002/api/v1/todos/${id}`);
